...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # The previous level is a state level
        s_level = self.s_levels[level]
        # Create new action level as set
        self.a_levels.append(set())
        # Iterate over "real actions" and "no-op actions"
        for action in self.all_actions:
            a_node = PgNode_a(action)  # create PgNode first to use the prenodes function
            if a_node.prenodes.issubset(s_level):  # all action´s preconditions in previous state level?
    ...
    def add_literal_level(self, level):
        ...
        # Create a new state/literal level as set
        self.s_levels.append(set())
        # Iterate over actions (a_node) from previous layer to determine which literal (s_node) to add
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        HINT: This set approach was discarded due to bad performance
        a1 = node_a1.action
        a2 = node_a2.action
        if set(a1.effect_add).intersection(set(a2.effect_rem)) or set(a2.effect_add).intersection(set(a1.effect_rem)):
            return True
        return False
        """
        # Is there an effect in action 1 that can be negated by an effect of action 2
        for expr in node_a1.action.effect_add:
            if expr in node_a2.action.effect_rem:
                return True
        # Is there an effect in action 2 that can be negated by an effect of action 1
        for expr in node_a2.action.effect_add:
            if expr in node_a1.action.effect_rem:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        # Check if a positive preconditon in action 1 is negated by an action 2 effect
        if set(a1.precond_pos).intersection(set(a2.effect_rem)):
                return True
        # Check if a negative precondition in action 1 is negated by an action 2 effect
        if set(a1.precond_neg).intersection(set(a2.effect_add)):
                return True
        # Check if a positive precondition in action 2 is negated by an action 1 effect
        if set(a2.precond_pos).intersection(set(a1.effect_rem)):
                return True
        # Check if a positive precondition in action 2 is negated by an action 1 effect
        if set(a2.precond_neg).intersection(set(a1.effect_add)):
                return True
        return False

        """
        # Check if any positive precond in action 1 is negated by some effect in action 2
        for expr in node_a1.action.precond_pos:
            if expr in node_a2.action.effect_rem:
                return True
        # Check if any negative precond in action 1 is negated by some effect in action 2
        for expr in node_a1.action.precond_neg:
            if expr in node_a2.action.effect_add:
                return True
        # Check if any positive precond in action 2 is negated by some effect in action 1
        for expr in node_a2.action.precond_pos:
            if expr in node_a1.action.effect_rem:
                return True
        # Check if any positive precond in action 2 is negated by some effect in action 1
        for expr in node_a2.action.precond_neg:
            if expr in node_a1.action.effect_add:
                return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # same expression but different bool value, we can´t use PgNode_s.__eq__  directly
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Iterate over all combinations of action nodes (states precondition)
        for node_a1 in node_s1.parents:
            for node_a2 in node_s2.parents:
                if not node_a1.is_mutex(node_a2):  # previously added by update_s_mutex()... mutexify
                    return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            is_goal = False
            # probably a partial goal will appear before last level
            for i, s_level in enumerate(self.s_levels):
                # bad performance with list comprehension, an early break is more useful
                # if goal in [s_node for s_node in self.s_levels[level] if s_node.is_pos]:
                for s_node in s_level:
